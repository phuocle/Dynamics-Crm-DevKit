# Implementation Plan: DefaultAzureCredential
...
Azure credential chain - tries multiple methods automatically

## Reference Implementation

See `CONNECTION_TYPES_PRIORITY_ORDER.md` for detailed implementation specifications.

From Rnwood.Dataverse.Data.PowerShell, the pattern for DefaultAzureCredential:
...
### New Files
- `v4/DynamicsCrm.DevKit.Shared/ConnectionBuilder/DefaultAzureCredentialConnectionBuilder.cs`
...
**Test File**: `v4/DynamicsCrm.DevKit.Shared.Tests/DefaultAzureCredentialConnectionTests.cs`

AI Prompt:
```
Create unit tests for DefaultAzureCredential connection:
...
```csharp
[TestClass]
public class DefaultAzureCredentialConnectionTests
{
    [TestMethod]
    public async Task CreateServiceClient_ValidConnection_Success()
    {
        ...
        var builder = new DefaultAzureCredentialConnectionBuilder();
...
**Running Tests**:
```powershell
dotnet test --filter "FullyQualifiedName~DefaultAzureCredentialConnection"
```
...
#### Test Scenario 1: Local dev with Azure CLI login

**Step 1**: Setup environment
- Configure DefaultAzureCredential in appropriate environment
- Verify prerequisites are met

**Step 2**: Create connection
1. Visual Studio → Tools → DynamicsCrm DevKit → Connect
2. Type: `DefaultAzureCredential`
...
#### Test Scenario 2: Azure pipeline with service connection

**Step 1**: Configure second scenario
- Setup Azure pipeline with service connection environment
