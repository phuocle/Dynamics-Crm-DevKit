...
namespace DynamicsCrm.DevKit.Shared
{
    public static class Helper
    {
        ...
        {
            // Bolt: Optimized using StringBuilder to avoid string concatenation in loop
        ...
        public static string GeneratorOptionSet(EntityMetadata EntityMetadata)
        {
            ...
            const string TAB = "\t";
            // Bolt: Optimized using StringBuilder to avoid excessive string allocations
            var sb = new StringBuilder();
            sb.Append($"/** @namespace OptionSet */{NEW_LINE}");
            sb.Append($"var OptionSet;{NEW_LINE}");
            sb.Append($"(function (OptionSet) {{{NEW_LINE}");
            sb.Append($"{TAB}OptionSet.{EntityMetadata.SchemaName} = {{{NEW_LINE}");
            foreach (var attribute in EntityMetadata.Attributes.OrderBy(x => x.SchemaName))
            {
                if (Helper.IsOptionSet(attribute))
                {
                    ...
                    if (values.Count == 0)
                    {
                        sb.Append($"{TAB}{TAB}{attribute.SchemaName} : {{ }},{NEW_LINE}");
                    }
                    else
                    {
                        sb.Append($"{TAB}{TAB}{attribute.SchemaName} : {{ ");
                        foreach (var value in values)
                        {
                            sb.Append($"{value.Name}: {value.Value}, ");
                        }
                        // Remove the last comma and space manually if needed, or handle it differently.
                        // Since we are using StringBuilder, we can remove from the end.
                        if (values.Count > 0)
                        {
                            sb.Length -= 2;
                        }
            ...
            sb.Append($"{TAB}{TAB}RollupState: {{ NotCalculated: 0, Calculated: 1, OverflowError: 2, OtherError: 3, RetryLimitExceeded: 4, HierarchicalRecursionLimitReached: 5, LoopDetected: 6 }}{NEW_LINE}");
            sb.Append($"{TAB}}};{NEW_LINE}");
            sb.Append($"}})(OptionSet || (OptionSet = {{}}));");
            return sb.ToString();
        }
        ...
        public static string GetDefaultHeaderForGeneratedCs()
        {
            ...
            var sb = new StringBuilder();
            sb.Append($"//---------------------------------------------------------------------------------------------------{NEW_LINE}");
            sb.Append($"// <auto-generated>{NEW_LINE}");
            sb.Append($"//{TAB}{TAB}Changes to this file may cause incorrect behavior and will be lost if the code is regenerated.{NEW_LINE}");
            sb.Append($"//{TAB}{TAB}Generated by DynamicsCrm.DevKit - https://github.com/phuocle/Dynamics-Crm-DevKit{NEW_LINE}");
            sb.Append($"//{TAB}{TAB}Last Modified: {DateTime.Now:yyyy-MM-dd HH:mm:ss}{NEW_LINE}");
            sb.Append($"// </auto-generated>{NEW_LINE}");
            sb.Append($"//---------------------------------------------------------------------------------------------------{NEW_LINE}");
            return sb.ToString();
        }
        ...
        public static string GetDefaultFileWithCs(EntityMetadata entityMetadata, string @namespace)
        {
            ...
            sb.Append($"using Microsoft.Xrm.Sdk;{NEW_LINE}");

            sb.Append($"using System;{NEW_LINE}");
            sb.Append(NEW_LINE);
            sb.Append($"namespace {@namespace}{NEW_LINE}");
            sb.Append($"{{{NEW_LINE}");
            sb.Append($"{TAB}public partial class {@class}{NEW_LINE}");
            sb.Append($"{TAB}{{{NEW_LINE}");
            sb.Append($"{TAB}{TAB}#region --- PROPERTIES ---{NEW_LINE}");
            sb.Append(NEW_LINE);
            sb.Append($"{TAB}{TAB}//public string StringField {{ get {{ return GetAliasedValue<string>(\"aliased.field\"); }} }}{NEW_LINE}");
            sb.Append($"{TAB}{TAB}//public int? IntField {{ get {{ return GetAliasedValue<int?>(\"aliased.field\"); }} }}{NEW_LINE}");
            sb.Append($"{TAB}{TAB}//public DateTime? DateTimeField {{ get {{ return GetAliasedValue<DateTime?>(\"aliased.field\"); }} }}{NEW_LINE}");
            sb.Append($"{TAB}{TAB}//public EntityReference LookupField {{ get {{ return GetAliasedValue<EntityReference>(\"aliased.field\"); }} }}{NEW_LINE}");
            sb.Append($"{TAB}{TAB}//public xxxOptionSets.xxx? OptionSetField {{ get {{ return (xxxOptionSets.xxx?)GetAliasedValue<OptionSetValue>(\"aliased.field\")?.Value; }} }}{NEW_LINE}");
            sb.Append($"{TAB}{TAB}//public decimal? MoneyField {{ get {{ return GetAliasedValue<Money>(\"aliased.field\")?.Value; }} }}{NEW_LINE}");
            sb.Append(NEW_LINE);
            sb.Append($"{TAB}{TAB}#endregion{NEW_LINE}");
            sb.Append(NEW_LINE);
            sb.Append($"{TAB}{TAB}#region --- STATIC METHODS ---{NEW_LINE}");
            sb.Append(NEW_LINE);
            sb.Append($"{TAB}{TAB}public static {@class} Read_Record(IOrganizationService serviceAdmin, IOrganizationService service, ITracingService tracing, Guid? recordId){NEW_LINE}");
            sb.Append($"{TAB}{TAB}{{{NEW_LINE}");
            sb.Append($"{TAB}{TAB}{TAB}var fetchData = new{NEW_LINE}");
            sb.Append($"{TAB}{TAB}{TAB}{{{NEW_LINE}");
            sb.Append($"{TAB}{TAB}{TAB}{TAB}{key} = recordId ?? Guid.Empty{NEW_LINE}");
            sb.Append($"{TAB}{TAB}{TAB}}};{NEW_LINE}");
            sb.Append($"{TAB}{TAB}{TAB}var fetchXml = $@\"{NEW_LINE}");
            sb.Append($"<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>{NEW_LINE}");
            sb.Append($"  <entity name='{@class.ToLower()}'>{NEW_LINE}");
            sb.Append($"    <all-attributes/>{NEW_LINE}");
            sb.Append($"    <filter type='and'>{NEW_LINE}");
            sb.Append($"      <condition attribute='{key}' operator='eq' value='{{fetchData.{key}}}'/>{NEW_LINE}");
            sb.Append($"    </filter>{NEW_LINE}");
            sb.Append($"  </entity>{NEW_LINE}");
            sb.Append($"</fetch>{NEW_LINE}");
            sb.Append($"\";{NEW_LINE}");
            sb.Append($"{TAB}{TAB}{TAB}var rows = serviceAdmin.RetrieveMultiple<{@class}>(fetchXml);{NEW_LINE}");
            sb.Append($"{TAB}{TAB}{TAB}if (rows.Count == 1) return rows[0];{NEW_LINE}");
            sb.Append($"{TAB}{TAB}{TAB}return new {@class}();{NEW_LINE}");
            sb.Append($"{TAB}{TAB}}}{NEW_LINE}");
            sb.Append(NEW_LINE);
