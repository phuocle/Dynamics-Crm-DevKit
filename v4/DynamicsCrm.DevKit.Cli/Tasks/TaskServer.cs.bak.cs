...
namespace DynamicsCrm.DevKit.Cli.Tasks
{
    public class TaskServer : ITask
    {
        ...
        public async Task RunAsync()
        {
            ...
            CliLog.WriteLine(ConsoleColor.White, "|");
            CliLog.WriteLine(ConsoleColor.White, "|", ConsoleColor.Cyan, $"Total Dataverse Requests: {XrmHelper.COUNT}");
  ...
  <entity name='managedidentity'>
    ...
    <filter type='and'>
      ...
      <condition attribute='tenantid' operator='eq' value='{TenantId}'/>
        ...
        private readonly Dictionary<string, EntityReference> _SdkMessageCache = new Dictionary<string, EntityReference>();
        private readonly Dictionary<string, EntityReference> _SdkMessageFilterCache = new Dictionary<string, EntityReference>();
        private readonly Dictionary<string, Guid> _ImpersonatingUserCache = new Dictionary<string, Guid>();
        private readonly Dictionary<Guid, Entity> _SecureConfigCache = new Dictionary<Guid, Entity>();
        private readonly Dictionary<string, int> _ObjectTypeCodeCache = new Dictionary<string, int>();
...
<fetch>
    <entity name='sdkmessageprocessingstepimage'>
        ...
        private async Task<int?> GetCachedObjectTypeCodeAsync(string entityName)
        {
            if (string.IsNullOrEmpty(entityName) || entityName.ToLower() == "none") return null;
            var key = entityName.ToLower();
            if (!_ObjectTypeCodeCache.ContainsKey(key))
            {
                var code = await XrmHelper.GetPrimaryObjectTypeCodeAsync(ServiceClient, entityName);
                if (code.HasValue)
                    _ObjectTypeCodeCache[key] = code.Value;
            }
            return _ObjectTypeCodeCache.ContainsKey(key) ? _ObjectTypeCodeCache[key] : (int?)null;
        }

        private async Task LoadAllSdkMessagesAndFiltersAsync(List<TypeInfo> types)
        {
            _SdkMessageCache.Clear();
            _SdkMessageFilterCache.Clear();
            _ImpersonatingUserCache.Clear();

            // Collect unique combinations from all attributes
            ...
            foreach (var type in types)
            {
                ...
                foreach (var attr in attributes)
                {
                    ...
                    if (!string.IsNullOrEmpty(attr.RunAs))
                    {
                        uniqueUsers.Add(attr.RunAs);
                    }
                }
            }

            // Batch load SDK Messages and Filters
            if (uniqueMessageCombinations.Count > 0)
            {
                ...
                for (int i = 0; i < messageBatches; i++)
                {
                    ...
                    var messageConditions = string.Empty;

                    foreach (var (entityLogicalName, message) in batch)
                    {
                        var objectTypeCode = await GetCachedObjectTypeCodeAsync(entityLogicalName);
        ...
        <filter type='and'>
            ...
            <condition attribute='primaryobjecttypecode' operator='eq' value='{objectTypeCode}'/>
                    ...
                    if (string.IsNullOrEmpty(messageConditions)) continue;
...
<fetch>
  <entity name='sdkmessagefilter'>
    ...
    <filter type='or'>{messageConditions}
    </filter>
  </entity>
</fetch>";
                    ...
                    {
                        var messageName = XrmHelper.GetAliasedValue<string>(row, "msg.name");
                        var messageId = XrmHelper.GetAliasedValue<Guid>(row, "msg.sdkmessageid");
                        var filterId = row.Id;
                        var objectTypeCode = row.GetAttributeValue<int>("primaryobjecttypecode");

                        var key = $"{objectTypeCode}-{messageName}";
                        if (!_SdkMessageCache.ContainsKey(key))
                            _SdkMessageCache[key] = new EntityReference("sdkmessage", messageId);
                        if (!_SdkMessageFilterCache.ContainsKey(key))
                            _SdkMessageFilterCache[key] = new EntityReference("sdkmessagefilter", filterId);
                    }
                }
            }

            // Batch load Impersonating Users
            if (uniqueUsers.Count > 0)
            {
                var userList = uniqueUsers.ToList();
                var userBatches = (int)Math.Ceiling((double)userList.Count / PACK);

                for (int i = 0; i < userBatches; i++)
                {
                    var batch = userList.Skip(i * PACK).Take(PACK).ToList();
                    var userConditions = string.Empty;
                    foreach (var userName in batch)
                        userConditions += $"<condition attribute='fullname' operator='eq' value='{userName}'/>";
...
<fetch>
  <entity name='systemuser'>
    ...
    <filter type='or'>{userConditions}</filter>
  </entity>
</fetch>";
                    ...
                    foreach (var row in rows)
                    {
                        var fullname = row.GetAttributeValue<string>("fullname");
                        if (!_ImpersonatingUserCache.ContainsKey(fullname))
                            _ImpersonatingUserCache[fullname] = row.Id;
                    }
                }
            }
        }

        private async Task LoadAllSecureConfigurationsAsync()
        {
            _SecureConfigCache.Clear();

            if (_PluginStepsCache.Count == 0) return;

            var stepIds = _PluginStepsCache.Select(x => x.Value.Id).ToList();
            ...
            for (int i = 0; i < batches; i++)
            {
                ...
                var condition = string.Empty;
                foreach (var stepId in batch)
                    condition += $"<condition attribute='sdkmessageprocessingstepid' operator='eq' value='{stepId}'/>";
  ...
  <entity name='sdkmessageprocessingstep'>
    ...
    <link-entity name='sdkmessageprocessingstepsecureconfig' from='sdkmessageprocessingstepsecureconfigid' to='sdkmessageprocessingstepsecureconfigid' link-type='outer' alias='s'>
      ...
      <attribute name='sdkmessageprocessingstepsecureconfigid' />
                ...
                foreach (var row in rows)
                {
                    if (!_SecureConfigCache.ContainsKey(row.Id))
                        _SecureConfigCache[row.Id] = row;
                }
            }
        }
        ...
        private async Task DeployFileAsync(string file, List<TypeInfo> types, DeployFileType deployFileType)
        {
            ...
            await LoadAllSdkMessagesAndFiltersAsync(sortedTypes);
            await LoadAllSecureConfigurationsAsync();
...
";
        ...
        private async Task<Guid?> DeployPluginStepAsync(Guid pluginTypeId, TypeInfo type, CrmPluginRegistrationAttribute attribute)
        {
            ...
            // Use cached values instead of individual calls
            var objectTypeCode = await GetCachedObjectTypeCodeAsync(attribute.EntityLogicalName);
            var cacheKey = objectTypeCode.HasValue ? $"{objectTypeCode.Value}-{attribute.Message}" : string.Empty;

            var sdkMessageFilterId = !string.IsNullOrEmpty(cacheKey) && _SdkMessageFilterCache.ContainsKey(cacheKey)
                ? _SdkMessageFilterCache[cacheKey]
                : await XrmHelper.GetSdkMessageFilterIdAsync(ServiceClient, attribute.EntityLogicalName, attribute.Message);
            var sdkMessageId = !string.IsNullOrEmpty(cacheKey) && _SdkMessageCache.ContainsKey(cacheKey)
                ? _SdkMessageCache[cacheKey]
                : await XrmHelper.GetSdkMessageIdAsync(ServiceClient, attribute.EntityLogicalName, attribute.Message);
            var impersonatingUserId = !string.IsNullOrEmpty(attribute.RunAs) && _ImpersonatingUserCache.ContainsKey(attribute.RunAs)
                ? (Guid?)_ImpersonatingUserCache[attribute.RunAs]
                : await XrmHelper.GetImpersonatingUserIdAsync(ServiceClient, attribute.RunAs);
            ...
            else
            {
                ...
                // Use cached secure entity instead of individual call
                var secureEntity = _SecureConfigCache.ContainsKey(pluginStepId.Value)
                    ? _SecureConfigCache[pluginStepId.Value]
                    : await XrmHelper.GetSecureEntityAsync(ServiceClient, pluginStepId.Value);
