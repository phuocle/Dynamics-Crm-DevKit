...
namespace PL.DevKit.Monkey.CustomAction._3.Test
{
    [TestClass]
    public class PostNonenew_AjaxSynchronousTest
    {
        ...
        [TestMethod]
        public void PostNonenew_Ajax_PrimaryEntityName_Does_Not_Equals_3()
        {
            var context = new XrmFakedContext();
            var plugin = context.GetDefaultPluginContext();
            plugin.Stage = (int)StageEnum.PostOperation;
            plugin.PrimaryEntityName = "abcd";
            Assert.ThrowsException<InvalidPluginExecutionException>(() => context.ExecutePluginWithConfigurations<PostNonenew_AjaxSynchronous>(plugin, null, null), "PrimaryEntityName does not equals 3");
        }
