...
> ```
> Error: /anti-optimize-tool-description requires 1 argument:
...
>   upsert_table, manage_form, upsert_column, upsert_relationship
>
> Usage: /anti-optimize-tool-description <tool_name>
> ```
...
> | **Apply** (Step 7) | Edit descriptions + error messages | `Edit` | **YES — only after user approval** |
>
> You **MUST NOT** use the `Edit` or `Write` tool during Steps 1-6.
> You **MUST** ask the user for approval at the start of Step 7 and WAIT for "Yes" before editing.
...
## Step 7: Apply Changes — ASK USER FIRST

> [!CAUTION]
> **MANDATORY GATE: You MUST ask the user for explicit approval BEFORE editing any file.**
...
Ask the user with this exact question:
...
After applying:
1. Run `/anti-build-cli` to verify compilation
2. Update this workflow file (move `{tool_name}` from `TODO` list to `DONE` list in the error message block at the top of this file)
3. Run `/anti-commit` to commit all changes
4. Report the result
...
- **NEVER edit any file without explicit user approval — this is the #1 rule of this workflow**
