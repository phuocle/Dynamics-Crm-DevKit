...
4. Verify the build:
   - Run `devkit --version`. Expected version format is `x.xx.xx.xx` (from `DevKit.ReleaseConfig.json`) with Build timestamp `31.03.2026 23:59:59` (exact date/time matching `DevKit.ReleaseConfig.json`).
