...
**Token hiện tại (description + parameters):** ~1109 tokens (`o200k_base`)
- Description: ~380 tokens (~1500 chars)
- 23 parameters: ~729 tokens (~1280 chars)
...
| Parameters (23) | ~729 tokens | ~210 tokens | -71% |
| **Tổng** | **~1109 tokens** | **~525 tokens** | **~53%** |

**Kết luận:** Tiết kiệm khoảng **53%** (~584 tokens, `o200k_base`). Restore WHEN TO USE + fuzzy disambiguation cho `label` & `app_name`. Giữ TOOL SELECTION + 12 actions + critical safety.
