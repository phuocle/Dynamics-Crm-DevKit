...
origin='default' excludes auto-migrated. Commands are app-scoped. CRITICAL: if error mentions 'classic ribbon button', STOP and report — don't call other tools.
```
...
|---|---:|---:|---:|
| Description | ~380 tokens | ~270 tokens | -29% |
| Parameters (23) | ~320 tokens | ~210 tokens | -34% |
| **Tổng** | **~700 tokens** | **~480 tokens** | **~31%** |

**Kết luận:** Tiết kiệm khoảng **31%** (~220 tokens). Description giảm vừa phải vì TOOL SELECTION + 12 actions + critical safety đều phải giữ.
