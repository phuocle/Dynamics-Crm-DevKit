...
**Token hiện tại (description + parameters):** ~307 tokens (`o200k_base`)
- Description: ~225 tokens (~895 chars)
- 6 parameters: ~82 tokens (~675 chars)
...
| Parameters (6) | ~82 tokens | ~70 tokens | -15% |
| **Tổng** | **~307 tokens** | **~235 tokens** | **~23%** |

**Kết luận:** Tiết kiệm khoảng **23%** (~72 tokens, `o200k_base`). Restore WHEN TO USE block riêng theo convention; giữ đầy đủ routing thông qua Blocked list.
