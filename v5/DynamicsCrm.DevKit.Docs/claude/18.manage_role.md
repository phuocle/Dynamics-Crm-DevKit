...
Depth: User < BU < Parent:ChildBU < Org. Only root roles listed (not BU-inherited copies).
```
...
|---|---:|---:|---:|
| Description | ~245 tokens | ~155 tokens | -37% |
| Parameters (7) | ~145 tokens | ~85 tokens | -41% |
| **Tổng** | **~390 tokens** | **~240 tokens** | **~38%** |

**Kết luận:** Tiết kiệm khoảng **38%** (~150 tokens). Giữ thứ tự action theo nghiệp vụ (read → write → delete).
