...
**Token hiện tại (description + parameters):** ~448 tokens (o200k_base)
...
| **Tổng** | **~448 tokens** | **~265 tokens** | **~41%** |

**Kết luận:** Tiết kiệm khoảng **41%** (~183 tokens). Restore WHEN TO USE block riêng + fuzzy disambiguation rule cho `role_name`.
