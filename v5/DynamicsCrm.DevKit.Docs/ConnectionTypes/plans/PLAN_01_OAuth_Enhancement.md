...
# 1. Build solution
# 2. Open Test Explorer (Test ΓåÆ Test Explorer)
# 3. Click "Run All"
...
- Visual Studio ΓåÆ Tools ΓåÆ DynamicsCrm DevKit ΓåÆ Connect
- Select existing connection from dropdown
- Click "Load"

**Expected Result**: Γ£à Connection loads successfully without errors

**Step 1.4**: Test connection
- Click "Test Connection" button
- Wait for authentication prompt

**Expected Result**: Γ£à Browser opens, authentication succeeds, connection established
...
**Expected Result**: Γ£à Deployment succeeds with no connection errors
...
1. Go to Azure Portal ΓåÆ Azure Active Directory ΓåÆ App registrations
...
**Expected Result**: Γ£à Browser opens with custom app consent screen, authentication succeeds
...
**Expected Result**: Γ£à Connection includes custom ClientId:
```json
{
  "Name": "CustomClient",
...
**Expected Result**: Γ£à Authentication uses custom app, deployment succeeds
...
```powershell
# Azure Portal ΓåÆ Azure Active Directory ΓåÆ Properties
...
**Expected Result**: Γ£à Authentication restricted to specified tenant only

**Step 3.3**: Test with wrong tenant user
1. Try to authenticate with user from different tenant (e.g., personal Microsoft account)

**Expected Result**: Γ£à Authentication fails with "AADSTS50020: User account from wrong tenant" or similar
...
**Expected Result**: Γ£à Old encryption decrypts successfully

**Step 4.2**: Re-save connection
1. Make minor change (e.g., add space to name, then remove it)
2. Click "Save"

**Expected Result**: Γ£à Password re-encrypted with modern AES

**Step 4.3**: Verify new encryption format
1. Close and reopen FormConnection
2. Load connection again

**Expected Result**: Γ£à Connection loads successfully with new encryption
...
**Expected Result**: Γ£à Connection saved, password NOT stored in JSON
...
**Expected Result**: Γ£à Password field is empty or not present:
```json
{
  "Name": "NoPassword",
  "Url": "...",
...
**Expected Result**: Γ£à Password field is empty, prompts for password on test/use
...
**Expected Result**: Γ£à Uses default Microsoft ClientId, authentication succeeds
...
**Expected Result**: Γ£à Uses custom ClientId, authentication succeeds
...
**Expected Result**: Γ£à Authentication restricted to specified tenant
