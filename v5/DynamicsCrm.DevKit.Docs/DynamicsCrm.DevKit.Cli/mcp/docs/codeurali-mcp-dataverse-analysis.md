...
| 6 | `manage_sitemap` | `create_sitemap` only (no update/undo) | MEDIUM — no edit or rollback |
...
| 47 | `dataverse_create_sitemap` | Solutions | MEDIUM | DevKit has `manage_sitemap` but no create |
...
| **SiteMap Lifecycle** | `manage_sitemap` (with backup/validate/undo) | App navigation management |
