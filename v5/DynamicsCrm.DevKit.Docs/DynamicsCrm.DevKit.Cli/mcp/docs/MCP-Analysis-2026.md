...
2. **No SiteMap Tool** — Despite having the SiteMap resource (schema://sitemapxml) and blocking SiteMap writes via `execute_webapi`, there is no dedicated `manage_sitemap` tool. It's marked as "coming soon" in the codebase.
...
- References "manage_sitemap (coming soon)" in the codebase
...
| Tool | Description |
|------|-------------|
| `manage_sitemap` | Read/update/undo SiteMap XML with backup + XSD validation + publish |
...
|---|--------|--------|--------|
| 1 | **Implement `manage_sitemap`** (already referenced as "coming soon") | Medium | 🔴 High |
