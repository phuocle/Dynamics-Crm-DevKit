...
| 6 | `get_solution_components` | ✅ true | ❌ false | Single | List all components inside a solution |
...
| 13 | `get_security_roles` | ✅ true | ❌ false | List / User / Detail | List roles, check user roles, or role detail |
| 14 | `get_plugin_trace_logs` | ✅ true | ❌ false | List / Detail | Query plugin trace logs (compact list or full detail) |
| 15 | `get_audit_histories` | ✅ true | ❌ false | Browse / Detail | Browse audit entries or detail for one record |
| 16 | `get_business_rules` | ✅ true | ❌ false | List / Detail | List business rules or get one rule's XAML |
...
| 3 | `manage_environment_variables` | ❌ false | ❌ false | **Mixed R/W** — `action=list|get` is read-only, `action=set` is write. Config changes affect all users and integrations. |
...
|---|---|---|
| **Returns a list (plural)** | Use plural noun | `get_forms`, `get_views`, `get_security_roles` |
| **Returns single detail (singular)** | Same tool, differentiated by parameter | `get_forms` + `form_id` → single form detail |
| **Tools with dual mode** | Same tool name handles both list and detail | `get_audit_histories` (browse mode vs detail mode via `record_id`) |
...
| 3 | `get_security_roles` | Lists all root security roles | Shows all privileges for one role | `role_id` or `user_id` | ✅ Yes — plural name, 3-mode switch |
| 4 | `get_plugin_trace_logs` | Compact list: metadata only (no trace output) | Returns FULL trace output + exception | `record_id` | ✅ Yes — plural name, param switches mode |
| 5 | `get_audit_histories` | Browse: summary across records/entities | Detail: field-level old/new values for one record | `record_id` | ✅ Yes — plural name, param switches mode |
| 6 | `get_business_rules` | Lists all rules: name, scope, status | Returns full XAML conditions+actions | `rule_id` | ✅ Yes — plural name, param switches mode |
...
| 5 | `get_solution_components` | Always a list of components | ✅ Plural — always returns list |
...
|---|---|---|---|---|---|
| 1 | `manage_environment_variables` | `list`, `get` | `set` | **advance** | Mixed R/W. `list`/`get` are pure read, `set` changes config for all users. |
...
private static readonly Dictionary<Type, string> ToolCategories = new()
{
    ...
    { typeof(GetSolutionComponentsTool), "read" },
    ...
    { typeof(GetSecurityRolesTool), "read" },
    { typeof(GetPluginTraceLogsTool), "read" },
    { typeof(GetAuditHistoriesTool), "read" },
    { typeof(GetBusinessRulesTool), "read" },
    ...
    { typeof(ManageEnvironmentVariablesTool), "advance" },
};
```
...
5. **Mixed-mode tools** → `manage_environment_variables` and `execute_webapi` are categorized as `advance` due to their mixed R/W nature. The `write` category tools (`update_form`, `update_view`, `update_sitemap`) have multiple write actions but no read-only actions.
