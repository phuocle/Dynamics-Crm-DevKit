...
> ✅ Tools: manage_webresource → manage_webresource
> Result: Found 5 web resources with "testmcp" (devkit_ prefix) and 1 with "invoice" (v4_/testmcp/invoice.form.js) — v5_ prefix web resource does not exist yet.
...
> ✅ Tools: manage_webresource
> Result: Created and published JS web resource v5_/testmcp/invoice.form.js (f2814c58-104a-f111-bec6-7ced8d6e68ce) in solution TESTMCP with namespace TestMcp.Invoice, syncLineStatus, and canSyncLineStatus functions.
...
> ✅ Tools: manage_webresource
> Result: Confirmed v5_/testmcp/invoice.form.js exists with namespace TestMcp.Invoice, syncLineStatus (reads v5_invoicestatus, updates all child v5_invoiceline records), and canSyncLineStatus (returns true only when record id is non-empty).
...
> ✅ Tools: manage_ribbon
> Result: Invoice form ribbon has 10 OOB buttons: Save, SaveAsComplete, SaveAndClose, SaveAndNew, New (x2), Activate, Deactivate, Delete, OpenActiveStage — no custom buttons yet.
...
> ✅ Tools: manage_ribbon → publish_customizations
> Result: Added "Sync Line Status" button to Invoice form ribbon (sequence 85) calling TestMcp.Invoice.syncLineStatus with enable rule TestMcp.Invoice.canSyncLineStatus from v5_/testmcp/invoice.form.js; published successfully.
...
> ✅ Tools: manage_ribbon
> Result: Confirmed custom button "Sync Line Status" (id: devkit.v5_invoice.SyncLineStatus.Form.Button, sequence 85) is present on the Invoice form ribbon, isCustom=true, isHide=false.
...
> ✅ Tools: manage_command → manage_command → manage_command
> Result: No modern appaction commands exist for v5_invoice form in any app (TEST-MCP or DEBUG-TEST-MCP); Activate and Deactivate are classic ribbon buttons, not modern command bar entries.
...
> ⚠️ Tools: manage_command
> Result: SKIPPED — Deactivate is a classic ribbon button (Mscrm.Form.v5_invoice.Deactivate), not a modern appaction command; hide via manage_command is not applicable and no ribbon substitute was made per prompt instructions.
...
> ⚠️ Tools: manage_command
> Result: SKIPPED — Activate is a classic ribbon button (Mscrm.Form.v5_invoice.Activate), not a modern appaction command; hide via manage_command is not applicable and no ribbon substitute was made per prompt instructions.
...
> ✅ Tools: manage_ribbon
> Result: Activate (sequence 50) and Deactivate (sequence 60) are both classic ribbon buttons with isHide=false — both currently visible; no modern appaction hide overrides exist.
...
> ⚠️ Tools: manage_command
> Result: SKIPPED — Deactivate is a classic ribbon button, not a modern appaction command; no appaction hide override record exists or can be created via manage_command.
...
> ⚠️ Tools: manage_command
> Result: SKIPPED — Activate is a classic ribbon button, not a modern appaction command; no appaction hide override record exists or can be created via manage_command.
...
> ✅ Tools: manage_record
> Result: Created Account "TEST-MCP Client A" with id 18934e97-1e4a-f111-bec6-7ced8d6e68ce.
