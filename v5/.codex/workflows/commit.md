...
2. Read `DynamicsCrm.DevKit.Scripts/DevKit.ReleaseConfig.json` and check every `files.dateReplacement` entry. Restore the source placeholder `xxxx.yy.zz HH.mm.ss` if a build left a real timestamp behind; keep stable version `4.12.34.56` unchanged.
