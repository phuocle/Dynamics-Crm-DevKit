...
4. Verify the build:
   - Run `devkit --version`. Expected version format is `x.xx.xx.xx` (from `DevKit.ReleaseConfig.json`) with Build timestamp in `dd.MM.yyyy HH:mm:ss` format (the exact current date/time when built). Confirm the date/month/year matches today.
