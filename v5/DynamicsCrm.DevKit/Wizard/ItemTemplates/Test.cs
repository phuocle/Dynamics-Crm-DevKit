...
{
    internal class Test : ItemTemplateBase, IWizard
    {
            ...
            using (TraceRunStarted())
            {
                ThreadHelper.JoinableTaskFactory.Run(async () =>
                {
                    ...
                    if (ok)
                    {
                        ...
                        var t4Code = await T4Helper.GetT4CodeAsync(ItemType.Test, templateTitle);
                        var t4Context = string.Equals(templateTitle, T4Helper.DefaultTemplateTitle, System.StringComparison.Ordinal)
                            ? await T4Helper.BuildTestContextAsync(form)
