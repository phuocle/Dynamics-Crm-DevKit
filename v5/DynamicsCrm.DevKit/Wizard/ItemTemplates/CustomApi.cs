...
{
    internal class CustomApi : ItemTemplateBase, IWizard
    {
        ...
        public void RunStarted(object automationObject, Dictionary<string, string> replacementsDictionary, WizardRunKind runKind, object[] customParams)
        {
            using (TraceRunStarted())
            {
                ...
                {
                    var form = new FormPlugin(ItemType.CustomApi, replacementsDictionary["$rootnamespace$"]);
                    ...
                    if (ok)
                    {
                        ...
                        var t4Code = await T4Helper.GetT4CodeAsync(ItemType.CustomApi, templateTitle);
