...
public class McpFormattingCoverageTests
{
    ...
    public void ManageView_BuildViewListEntries_MapsSystemAndPersonalViews()
    {
        ...
        var entries = InvokeStatic<System.Collections.IEnumerable>(ManageViewType, "BuildViewListEntries",
            systemViews, personalViews).Cast<object>().ToList();

        Assert.AreEqual(2, entries.Count);
        Assert.AreEqual("Active\tAccounts", GetProperty<string>(entries[0], "ViewName"));
        Assert.AreEqual("system", GetProperty<string>(entries[0], "Source"));
        Assert.AreEqual("My Accounts", GetProperty<string>(entries[1], "ViewName"));
        Assert.AreEqual("personal", GetProperty<string>(entries[1], "Source"));
    }
    ...
    public void ManageView_ValidationAndCellUpdateHelpers_ReturnDetailedErrors()
    {
        ...
        var blocked = InvokeStatic<object>(ManageViewType, "BuildBlockedValidationDto",
            "update", "account", Guid.NewGuid(), "Active Accounts",
            new List<string> { "layout missing cell" },
            new List<string> { "schema warning" });
        Assert.AreEqual("blocked_validation", GetProperty<string>(blocked, "Status"));
        var validationErrors = GetProperty<List<string>>(blocked, "ValidationErrors");
        CollectionAssert.Contains(validationErrors, "layout missing cell");
        CollectionAssert.Contains(validationErrors, "schema warning");
