...
""";
    ...
    // ──────────────────────────────────────────────
    // Current form list formatter
    // ──────────────────────────────────────────────

    private static readonly MethodInfo BuildFormListEntriesMethod = ToolType
        .GetMethod("BuildFormListEntries", BindingFlags.NonPublic | BindingFlags.Static)!;

    [TestMethod]
    public void BuildFormListEntries_MapsCurrentFormMetadata()
    {
        var formId = Guid.NewGuid();
        var forms = new EntityCollection().Entities;
        forms.Add(new Entity("systemform")
        {
            ["formid"] = formId,
            ["name"] = "Main Form",
        ...
        var entries = (System.Collections.IList)BuildFormListEntriesMethod.Invoke(null,
            new object[] { forms, true })!;

        Assert.AreEqual(1, entries.Count);
        var entry = entries[0]!;
        Assert.AreEqual(formId.ToString(), entry.GetType().GetProperty("FormId")!.GetValue(entry));
        Assert.AreEqual("Main", entry.GetType().GetProperty("FormTypeName")!.GetValue(entry));
        Assert.AreEqual("<form />", entry.GetType().GetProperty("FormXml")!.GetValue(entry));
    }
    ...
    [TestMethod]
    public void GetFormDetailResult_MethodExists_AsInstanceHelper()
    {
        var method = ToolType.GetMethod("GetFormDetailResult", BindingFlags.NonPublic | BindingFlags.Instance);
        Assert.IsNotNull(method, "GetFormDetailResult helper must exist");
        Assert.AreEqual(typeof(ModelContextProtocol.Protocol.CallToolResult), method.ReturnType);
    }
