...
/// </summary>
[TestClass]
public class GetAuditHistoryToolTests
{
    ...
    // ──────────────────────────────────────────────
    // BuildBrowseText (private static)
    // ──────────────────────────────────────────────

    private static readonly MethodInfo BuildBrowseTextMethod = ToolType
        .GetMethod("BuildBrowseText", BindingFlags.NonPublic | BindingFlags.Static)!;

    private static string BuildBrowseText(string entityName, string timeScope, string userFilter, string operation, int count)
    {
        return (string)BuildBrowseTextMethod.Invoke(null,
            new object[] { entityName, timeScope, userFilter, operation, count })!;
    }

    [TestMethod]
    public void BuildBrowseText_Basic_ShowsZeroEntries()
    {
        var result = BuildBrowseText("", "last 24h", "", "", 0);

        Assert.AreEqual("all entities (last 24h): 0 entries.", result);
    }

    [TestMethod]
    public void BuildBrowseText_WithFilters_ShowsFilterDetails()
    {
        var result = BuildBrowseText("account", "last 24h", "John", "Create", 0);

        Assert.IsTrue(result.StartsWith("account (last 24h"));
        Assert.IsTrue(result.Contains("user contains \"John\""));
        Assert.IsTrue(result.Contains("op=Create"));
    }
