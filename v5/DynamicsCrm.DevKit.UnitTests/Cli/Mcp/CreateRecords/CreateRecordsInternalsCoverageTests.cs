...
public class CreateRecordsInternalsCoverageTests
{
    ...
    public void ResolveRecordsInput_InlineJsonAndJsonFile_ReturnContent()
    {
        ...
        var inline = ((string json, string format))InvokeInstance(tool, "ResolveRecordsInput", "[{\"name\":\"A\"}]", "account", warnings)!;
        Assert.AreEqual("[{\"name\":\"A\"}]", inline.json);

        var path = Path.Combine(Path.GetTempPath(), $"records-{Guid.NewGuid():N}.json");
        File.WriteAllText(path, "[{\"name\":\"B\"}]");
        var fileContent = ((string json, string format))InvokeInstance(tool, "ResolveRecordsInput", path, "account", warnings)!;
        Assert.AreEqual("[{\"name\":\"B\"}]", fileContent.json);
        Assert.IsTrue(File.Exists(path), "Input json files are preserved after read.");

        File.Delete(path);
        var missing = ((string json, string format))InvokeInstance(tool, "ResolveRecordsInput", path, "account", warnings)!;
        Assert.IsNull(missing.json);

        var emptyCsv = Path.Combine(Path.GetTempPath(), $"records-{Guid.NewGuid():N}.csv");
        File.WriteAllText(emptyCsv, "Name");
        var emptyCsvJson = ((string json, string format))InvokeInstance(tool, "ResolveRecordsInput", emptyCsv, "account", warnings)!;
        Assert.AreEqual("[]", emptyCsvJson.json);
        Assert.IsTrue(warnings.Exists(w => w.Contains("no data rows")));
        Assert.IsTrue(File.Exists(emptyCsv), "Input csv files are preserved after read.");
        ...
        var noMetadataJson = ((string json, string format))InvokeInstance(tool, "ResolveRecordsInput", csvWithoutMetadata, "account", warnings)!;
        Assert.AreEqual("[]", noMetadataJson.json);
        Assert.IsTrue(warnings.Exists(w => w.Contains("Failed to load metadata")));
        File.Delete(csvWithoutMetadata);
    }
    ...
    private static void AssertValue(CliCreateRecordsTool tool, AttributeMetadata attr, string cellValue, string logicalName,
    ...
    private static object? InvokeConvert(CliCreateRecordsTool tool, AttributeMetadata attr, string cellValue, string logicalName,
