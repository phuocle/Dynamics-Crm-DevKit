...
2. Run:
   ...
   ```
   The script explicitly discovers and forcefully kills any running `DynamicsCrm.DevKit.Cli` and `devkit` processes, including a running MCP server, before building to avoid file-lock errors. It builds the release package outputs in Release mode without running unit tests. The version comes from `DevKit.ReleaseConfig.json`, while the date/time is overridden with the current date/time.
...
5. Verify the build:
   ...
   - `devkit --version` should show version `x.xx.xx.xx` from `DevKit.ReleaseConfig.json` and a Build timestamp in `dd.MM.yyyy HH:mm:ss` format for today's date.
