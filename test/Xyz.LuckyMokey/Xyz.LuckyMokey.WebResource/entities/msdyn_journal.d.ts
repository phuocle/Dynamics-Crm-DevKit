//@ts-check
///<reference path="DevKit.d.ts" />
declare namespace LuckyMokey {
	namespace Formmsdyn_journal_Information {
		interface tab__89B13559_B90E_4EAA_A6BE_47E4EBB36524_Sections {
			_89B13559_B90E_4EAA_A6BE_47E4EBB36524_SECTION_2: DevKit.Form.Controls.ControlSection;
		}
		interface tab_TimeEntryCorrectionsTab_Sections {
			TimeEntryCorrectionParameters: DevKit.Form.Controls.ControlSection;
			tab_3_section_1: DevKit.Form.Controls.ControlSection;
		}
		interface tab_ExpenseCorrectionsTab_Sections {
			ExpenseCorrectionParameters: DevKit.Form.Controls.ControlSection;
			tab_4_section_1: DevKit.Form.Controls.ControlSection;
		}
		interface tab_JounalLinesTab_Sections {
			JournalLinesSection: DevKit.Form.Controls.ControlSection;
		}
		interface tab__89B13559_B90E_4EAA_A6BE_47E4EBB36524 extends DevKit.Form.Controls.IControlTab {
			Section: tab__89B13559_B90E_4EAA_A6BE_47E4EBB36524_Sections;
		}
		interface tab_TimeEntryCorrectionsTab extends DevKit.Form.Controls.IControlTab {
			Section: tab_TimeEntryCorrectionsTab_Sections;
		}
		interface tab_ExpenseCorrectionsTab extends DevKit.Form.Controls.IControlTab {
			Section: tab_ExpenseCorrectionsTab_Sections;
		}
		interface tab_JounalLinesTab extends DevKit.Form.Controls.IControlTab {
			Section: tab_JounalLinesTab_Sections;
		}
		interface Tabs {
			_89B13559_B90E_4EAA_A6BE_47E4EBB36524: tab__89B13559_B90E_4EAA_A6BE_47E4EBB36524;
			TimeEntryCorrectionsTab: tab_TimeEntryCorrectionsTab;
			ExpenseCorrectionsTab: tab_ExpenseCorrectionsTab;
			JounalLinesTab: tab_JounalLinesTab;
		}
		interface Body {
			Tab: Tabs;
			TimeEntries: DevKit.Form.Controls.ControlGrid;
			Expenses: DevKit.Form.Controls.ControlGrid;
			JournalLinesGrid: DevKit.Form.Controls.ControlGrid;
			/** Bookable Resource */
			msdyn_BookableResource: DevKit.Form.Controls.ControlLookup;
			/** Bookable Resource */
			msdyn_BookableResource_1: DevKit.Form.Controls.ControlLookup;
			/** Time Entry Date */
			msdyn_Date: DevKit.Form.Controls.ControlDate;
			/** The name of the custom entity. */
			msdyn_description: DevKit.Form.Controls.ControlString;
			/** Expense Category */
			msdyn_ExpenseCategory: DevKit.Form.Controls.ControlLookup;
			/** Shows if the journal has been submitted. */
			msdyn_IsPosted: DevKit.Form.Controls.ControlBoolean;
			/** The type of the journal. */
			msdyn_JournalType: DevKit.Form.Controls.ControlOptionSet;
			/** Project */
			msdyn_Project: DevKit.Form.Controls.ControlLookup;
			/** Project */
			msdyn_Project_1: DevKit.Form.Controls.ControlLookup;
			/** Project Task */
			msdyn_ProjectTask: DevKit.Form.Controls.ControlLookup;
			/** Resource Role */
			msdyn_ResourceCategory: DevKit.Form.Controls.ControlLookup;
			/** Expense Date */
			msdyn_TransactionDate: DevKit.Form.Controls.ControlDate;
			/** Owner Id */
			OwnerId: DevKit.Form.Controls.ControlLookup;
			/** Reason for the status of the Journal */
			statuscode: DevKit.Form.Controls.ControlOptionSet;
		}
	}
	class Formmsdyn_journal_Information extends DevKit.Form.IForm {
		/**
		* DynamicsCrm.DevKit form msdyn_journal_Information
		* @param executionContext the execution context
		* @param defaultWebResourceName default resource name. E.g.: "devkit_/resources/Resource"
		*/
		constructor(executionContext: any, defaultWebResourceName?: string);
		/** Utility functions/methods/objects for Dynamics 365 form */
		Utility: DevKit.Form.Utility;
		/** Provides properties and methods to use Web API to create and manage records and execute Web API actions and functions in Customer Engagement */
		WebApi: DevKit.Form.WebApi;
		/** The Body section of form msdyn_journal_Information */
		Body: LuckyMokey.Formmsdyn_journal_Information.Body;
	}
}
declare namespace OptionSet {
	namespace msdyn_journal {
		enum msdyn_JournalType {
			/** 192350000 */
			Entry,
			/** 192350001 */
			Time_Correction,
			/** 192350002 */
			Expense_Correction
		}
		enum statecode {
			/** 0 */
			Active,
			/** 1 */
			Inactive
		}
		enum statuscode {
			/** 1 */
			Active,
			/** 192350000 */
			Draft_Adjustment,
			/** 2 */
			Inactive
		}
        enum RollupState {
            /** 0 - Attribute value is yet to be calculated */
            NotCalculated,
            /** 1 - Attribute value has been calculated per the last update time in <AttributeSchemaName>_Date attribute */
            Calculated,
            /** 2 - Attribute value calculation lead to overflow error */
            OverflowError,
            /** 3 - Attribute value calculation failed due to an internal error, next run of calculation job will likely fix it */
            OtherError,
            /** 4 - Attribute value calculation failed because the maximum number of retry attempts to calculate the value were exceeded likely due to high number of concurrency and locking conflicts */
            RetryLimitExceeded,
            /** 5 - Attribute value calculation failed because maximum hierarchy depth limit for calculation was reached */
            HierarchicalRecursionLimitReached,
            /** 6 - Attribute value calculation failed because a recursive loop was detected in the hierarchy of the record */
            LoopDetected
        }
	}
}
//{'JsForm':['Information'],'JsWebApi':false,'IsDebugForm':true,'IsDebugWebApi':false}