//---------------------------------------------------------------------------------------------------
// <auto-generated>
//		Changes to this file may cause incorrect behavior and will be lost if the code is regenerated.
//		Generated by DynamicsCrm.DevKit - https://github.com/phuocle/Dynamics-Crm-DevKit
// </auto-generated>
//---------------------------------------------------------------------------------------------------
using Microsoft.Xrm.Sdk;
using System;
using System.Diagnostics;

namespace Xyz.LuckyMokey.Shared.Entities.AuditOptionSets
{
	public enum Action
	{
		/// <summary>
		/// Unknown = 0
		/// </summary>
		Unknown = 0,
		/// <summary>
		/// Create = 1
		/// </summary>
		Create = 1,
		/// <summary>
		/// Update = 2
		/// </summary>
		Update = 2,
		/// <summary>
		/// Delete = 3
		/// </summary>
		Delete = 3,
		/// <summary>
		/// Activate = 4
		/// </summary>
		Activate = 4,
		/// <summary>
		/// Deactivate = 5
		/// </summary>
		Deactivate = 5,
		/// <summary>
		/// Cascade = 11
		/// </summary>
		Cascade = 11,
		/// <summary>
		/// Merge = 12
		/// </summary>
		Merge = 12,
		/// <summary>
		/// Assign = 13
		/// </summary>
		Assign = 13,
		/// <summary>
		/// Share = 14
		/// </summary>
		Share = 14,
		/// <summary>
		/// Retrieve = 15
		/// </summary>
		Retrieve = 15,
		/// <summary>
		/// Close = 16
		/// </summary>
		Close = 16,
		/// <summary>
		/// Cancel = 17
		/// </summary>
		Cancel = 17,
		/// <summary>
		/// Complete = 18
		/// </summary>
		Complete = 18,
		/// <summary>
		/// Resolve = 20
		/// </summary>
		Resolve = 20,
		/// <summary>
		/// Reopen = 21
		/// </summary>
		Reopen = 21,
		/// <summary>
		/// Fulfill = 22
		/// </summary>
		Fulfill = 22,
		/// <summary>
		/// Paid = 23
		/// </summary>
		Paid = 23,
		/// <summary>
		/// Qualify = 24
		/// </summary>
		Qualify = 24,
		/// <summary>
		/// Disqualify = 25
		/// </summary>
		Disqualify = 25,
		/// <summary>
		/// Submit = 26
		/// </summary>
		Submit = 26,
		/// <summary>
		/// Reject = 27
		/// </summary>
		Reject = 27,
		/// <summary>
		/// Approve = 28
		/// </summary>
		Approve = 28,
		/// <summary>
		/// Invoice = 29
		/// </summary>
		Invoice = 29,
		/// <summary>
		/// Hold = 30
		/// </summary>
		Hold = 30,
		/// <summary>
		/// Add_Member = 31
		/// </summary>
		Add_Member = 31,
		/// <summary>
		/// Remove_Member = 32
		/// </summary>
		Remove_Member = 32,
		/// <summary>
		/// Associate_Entities = 33
		/// </summary>
		Associate_Entities = 33,
		/// <summary>
		/// Disassociate_Entities = 34
		/// </summary>
		Disassociate_Entities = 34,
		/// <summary>
		/// Add_Members = 35
		/// </summary>
		Add_Members = 35,
		/// <summary>
		/// Remove_Members = 36
		/// </summary>
		Remove_Members = 36,
		/// <summary>
		/// Add_Item = 37
		/// </summary>
		Add_Item = 37,
		/// <summary>
		/// Remove_Item = 38
		/// </summary>
		Remove_Item = 38,
		/// <summary>
		/// Add_Substitute = 39
		/// </summary>
		Add_Substitute = 39,
		/// <summary>
		/// Remove_Substitute = 40
		/// </summary>
		Remove_Substitute = 40,
		/// <summary>
		/// Set_State = 41
		/// </summary>
		Set_State = 41,
		/// <summary>
		/// Renew = 42
		/// </summary>
		Renew = 42,
		/// <summary>
		/// Revise = 43
		/// </summary>
		Revise = 43,
		/// <summary>
		/// Win = 44
		/// </summary>
		Win = 44,
		/// <summary>
		/// Lose = 45
		/// </summary>
		Lose = 45,
		/// <summary>
		/// Internal_Processing = 46
		/// </summary>
		Internal_Processing = 46,
		/// <summary>
		/// Reschedule = 47
		/// </summary>
		Reschedule = 47,
		/// <summary>
		/// Modify_Share = 48
		/// </summary>
		Modify_Share = 48,
		/// <summary>
		/// Unshare = 49
		/// </summary>
		Unshare = 49,
		/// <summary>
		/// Book = 50
		/// </summary>
		Book = 50,
		/// <summary>
		/// Generate_Quote_From_Opportunity = 51
		/// </summary>
		Generate_Quote_From_Opportunity = 51,
		/// <summary>
		/// Add_To_Queue = 52
		/// </summary>
		Add_To_Queue = 52,
		/// <summary>
		/// Assign_Role_To_Team = 53
		/// </summary>
		Assign_Role_To_Team = 53,
		/// <summary>
		/// Remove_Role_From_Team = 54
		/// </summary>
		Remove_Role_From_Team = 54,
		/// <summary>
		/// Assign_Role_To_User = 55
		/// </summary>
		Assign_Role_To_User = 55,
		/// <summary>
		/// Remove_Role_From_User = 56
		/// </summary>
		Remove_Role_From_User = 56,
		/// <summary>
		/// Add_Privileges_to_Role = 57
		/// </summary>
		Add_Privileges_to_Role = 57,
		/// <summary>
		/// Remove_Privileges_From_Role = 58
		/// </summary>
		Remove_Privileges_From_Role = 58,
		/// <summary>
		/// Replace_Privileges_In_Role = 59
		/// </summary>
		Replace_Privileges_In_Role = 59,
		/// <summary>
		/// Import_Mappings = 60
		/// </summary>
		Import_Mappings = 60,
		/// <summary>
		/// Clone = 61
		/// </summary>
		Clone = 61,
		/// <summary>
		/// Send_Direct_Email = 62
		/// </summary>
		Send_Direct_Email = 62,
		/// <summary>
		/// Enabled_for_organization = 63
		/// </summary>
		Enabled_for_organization = 63,
		/// <summary>
		/// User_Access_via_Web = 64
		/// </summary>
		User_Access_via_Web = 64,
		/// <summary>
		/// User_Access_via_Web_Services = 65
		/// </summary>
		User_Access_via_Web_Services = 65,
		/// <summary>
		/// Delete_Entity = 100
		/// </summary>
		Delete_Entity = 100,
		/// <summary>
		/// Delete_Attribute = 101
		/// </summary>
		Delete_Attribute = 101,
		/// <summary>
		/// Audit_Change_at_Entity_Level = 102
		/// </summary>
		Audit_Change_at_Entity_Level = 102,
		/// <summary>
		/// Audit_Change_at_Attribute_Level = 103
		/// </summary>
		Audit_Change_at_Attribute_Level = 103,
		/// <summary>
		/// Audit_Change_at_Org_Level = 104
		/// </summary>
		Audit_Change_at_Org_Level = 104,
		/// <summary>
		/// Entity_Audit_Started = 105
		/// </summary>
		Entity_Audit_Started = 105,
		/// <summary>
		/// Attribute_Audit_Started = 106
		/// </summary>
		Attribute_Audit_Started = 106,
		/// <summary>
		/// Audit_Enabled = 107
		/// </summary>
		Audit_Enabled = 107,
		/// <summary>
		/// Entity_Audit_Stopped = 108
		/// </summary>
		Entity_Audit_Stopped = 108,
		/// <summary>
		/// Attribute_Audit_Stopped = 109
		/// </summary>
		Attribute_Audit_Stopped = 109,
		/// <summary>
		/// Audit_Disabled = 110
		/// </summary>
		Audit_Disabled = 110,
		/// <summary>
		/// Audit_Log_Deletion = 111
		/// </summary>
		Audit_Log_Deletion = 111,
		/// <summary>
		/// User_Access_Audit_Started = 112
		/// </summary>
		User_Access_Audit_Started = 112,
		/// <summary>
		/// User_Access_Audit_Stopped = 113
		/// </summary>
		User_Access_Audit_Stopped = 113
	}

	public enum Operation
	{
		/// <summary>
		/// Create = 1
		/// </summary>
		Create = 1,
		/// <summary>
		/// Update = 2
		/// </summary>
		Update = 2,
		/// <summary>
		/// Delete = 3
		/// </summary>
		Delete = 3,
		/// <summary>
		/// Access = 4
		/// </summary>
		Access = 4
	}
}

namespace Xyz.LuckyMokey.Shared.Entities
{
	public partial class Audit : EntityBase
	{
		public struct Fields
		{
			public const string Action = "action";
			public const string AttributeMask = "attributemask";
			public const string AuditId = "auditid";
			public const string CallingUserId = "callinguserid";
			public const string ChangeData = "changedata";
			public const string CreatedOn = "createdon";
			public const string ObjectId = "objectid";
			public const string Operation = "operation";
			public const string RegardingObjectId = "regardingobjectid";
			public const string TransactionId = "transactionid";
			public const string UserAdditionalInfo = "useradditionalinfo";
			public const string UserId = "userid";
		}

		public const string EntityLogicalName = "audit";

		public const int EntityTypeCode = 4567;

		[DebuggerNonUserCode()]
		public Audit()
		{
			Entity = new Entity(EntityLogicalName);
			PreEntity = CloneThisEntity(Entity);
		}

		[DebuggerNonUserCode()]
		public Audit(Guid AuditId)
		{
			Entity = new Entity(EntityLogicalName, AuditId);
			PreEntity = CloneThisEntity(Entity);
		}

		[DebuggerNonUserCode()]
		public Audit(string keyName, object keyValue)
		{
			Entity = new Entity(EntityLogicalName, keyName, keyValue);
			PreEntity = CloneThisEntity(Entity);
		}

		[DebuggerNonUserCode()]
		public Audit(Entity entity)
		{
			Entity = entity;
			PreEntity = CloneThisEntity(Entity);
		}

		[DebuggerNonUserCode()]
		public Audit(Entity entity, Entity merge)
		{
			Entity = entity;
			foreach (var property in merge?.Attributes)
			{
				var key = property.Key;
				var value = property.Value;
				Entity[key] = value;
			}
			PreEntity = CloneThisEntity(Entity);
		}

		[DebuggerNonUserCode()]
		public Audit(KeyAttributeCollection keys)
		{
			Entity = new Entity(EntityLogicalName, keys);
			PreEntity = CloneThisEntity(Entity);
		}

		/// <summary>
		/// <para>Actions the user can perform that cause a change</para>
		/// <para>ReadOnly - Required - Picklist</para>
		/// <para>Event</para>
		/// </summary>
		[DebuggerNonUserCode()]
		public Xyz.LuckyMokey.Shared.Entities.AuditOptionSets.Action? Action
		{
			get
			{
				var value = Entity.GetAttributeValue<OptionSetValue>(Fields.Action);
				if (value == null) return null;
				return (Xyz.LuckyMokey.Shared.Entities.AuditOptionSets.Action)value.Value;
			}
		}

		/// <summary>
		/// <para>Contains a CSV of the ColumnNumber metadata property of attributes</para>
		/// <para>ReadOnly - Memo - MaxLength: 2000</para>
		/// <para>Changed Field</para>
		/// </summary>
		[DebuggerNonUserCode()]
		public string AttributeMask
		{
			get { return Entity.GetAttributeValue<string>(Fields.AttributeMask); }
		}

		/// <summary>
		/// <para>Unique identifier of the auditing instance</para>
		/// <para>ReadOnly - Primary Key - Uniqueidentifier</para>
		/// <para>Record Id</para>
		/// </summary>
		[DebuggerNonUserCode()]
		public Guid AuditId
		{
			get { return Id; }
		}

		/// <summary>
		/// <para>Unique identifier of the calling user in case of an impersonated call</para>
		/// <para>ReadOnly - Lookup</para>
		/// <para>Calling User</para>
		/// </summary>
		[DebuggerNonUserCode()]
		public EntityReference CallingUserId
		{
			get { return Entity.GetAttributeValue<EntityReference>(Fields.CallingUserId); }
		}

		/// <summary>
		/// <para>Contains a CSV of old values of all the attributes whose IsAuditEnabled property is True and are being changed</para>
		/// <para>ReadOnly - Memo - MaxLength: 2000</para>
		/// <para>Change Data</para>
		/// </summary>
		[DebuggerNonUserCode()]
		public string ChangeData
		{
			get { return Entity.GetAttributeValue<string>(Fields.ChangeData); }
		}

		/// <summary>
		/// <para>Date and time when the audit record was created.</para>
		/// <para>ReadOnly - DateTimeBehavior: UserLocal - DateTimeFormat: DateAndTime</para>
		/// <para>Changed Date</para>
		/// </summary>
		[DebuggerNonUserCode()]
		public DateTime? CreatedOnUtc
		{
			get { return Entity.GetAttributeValue<DateTime?>(Fields.CreatedOn); }
		}

		/// <summary>
		/// <para>Unique identifier of the record that is being audited</para>
		/// <para>ReadOnly - Lookup</para>
		/// <para>Record</para>
		/// </summary>
		[DebuggerNonUserCode()]
		public EntityReference ObjectId
		{
			get { return Entity.GetAttributeValue<EntityReference>(Fields.ObjectId); }
		}

		/// <summary>
		/// <para>The action that causes the audit--it will be create, delete, or update</para>
		/// <para>ReadOnly - Picklist</para>
		/// <para>Operation</para>
		/// </summary>
		[DebuggerNonUserCode()]
		public Xyz.LuckyMokey.Shared.Entities.AuditOptionSets.Operation? Operation
		{
			get
			{
				var value = Entity.GetAttributeValue<OptionSetValue>(Fields.Operation);
				if (value == null) return null;
				return (Xyz.LuckyMokey.Shared.Entities.AuditOptionSets.Operation)value.Value;
			}
		}

		/// <summary>
		/// <para>Unique identifier of the object with which the record is associated.</para>
		/// <para>Lookup</para>
		/// <para>Regarding</para>
		/// </summary>
		[DebuggerNonUserCode()]
		public EntityReference RegardingObjectId
		{
			get { return Entity.GetAttributeValue<EntityReference>(Fields.RegardingObjectId); }
			set { Entity.Attributes[Fields.RegardingObjectId] = value; }
		}

		/// <summary>
		/// <para>Unique identifier for multiple changes that are part of a single operation; this field contains the same GUID for all the audit rows generated in a single transaction</para>
		/// <para>ReadOnly - Uniqueidentifier</para>
		/// <para>Transaction Id</para>
		/// </summary>
		[DebuggerNonUserCode()]
		public Guid? TransactionId
		{
			get { return Entity.GetAttributeValue<Guid?>(Fields.TransactionId); }
		}

		/// <summary>
		/// <para>Additional information associated to the user who caused the change.</para>
		/// <para>String - MaxLength: 350</para>
		/// <para>User Info</para>
		/// </summary>
		[DebuggerNonUserCode()]
		public string UserAdditionalInfo
		{
			get { return Entity.GetAttributeValue<string>(Fields.UserAdditionalInfo); }
			set { Entity.Attributes[Fields.UserAdditionalInfo] = value; }
		}

		/// <summary>
		/// <para>Unique identifier of the user who caused a change</para>
		/// <para>ReadOnly - Lookup</para>
		/// <para>Changed By</para>
		/// </summary>
		[DebuggerNonUserCode()]
		public EntityReference UserId
		{
			get { return Entity.GetAttributeValue<EntityReference>(Fields.UserId); }
		}
	}
}
