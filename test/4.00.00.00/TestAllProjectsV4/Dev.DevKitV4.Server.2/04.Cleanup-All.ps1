...
Write-Host "⚠️  CLEANUP SCRIPT - DELETE ALL RESOURCES" -ForegroundColor Yellow
...
Write-Host "📋 The following resources will be DELETED:" -ForegroundColor Cyan
Write-Host ""

if ($config) {
    if ($config.AppName) {
        Write-Host "  🗑️  App Registration: $($config.AppName)" -ForegroundColor White
        Write-Host "     • AppId: $($config.AppId)" -ForegroundColor Gray
        Write-Host "     • All federated credentials" -ForegroundColor Gray
    }

    if ($config.ResourceGroup) {
        Write-Host "  🗑️  Resource Group: $($config.ResourceGroup)" -ForegroundColor White
    ...
    if ($config.CertificateThumbprint) {
        Write-Host "  🗑️  Certificate from Windows Store:" -ForegroundColor White
...
Write-Host "  🗑️  Local Files:" -ForegroundColor White
...
if ($config -and ($config.AppId -or $config.ResourceGroup)) {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            Write-Host "❌ Failed to login to Azure. Skipping Azure cleanup.`n" -ForegroundColor Red
        }
        else {
    ...
    if ($config.AppId) {
        Write-Host "🗑️  Deleting App Registration..." -ForegroundColor Yellow
        Write-Host "   AppId: $($config.AppId)" -ForegroundColor Gray
        ...
        try {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                if ($LASTEXITCODE -eq 0) {
                    ...
                    Write-Host "   ❌ Failed to delete App Registration`n" -ForegroundColor Red
                }
            }
            else {
                Write-Host "   ℹ️  App Registration not found (may have been already deleted)`n" -ForegroundColor Yellow
            }
        }
        catch {
        ...
        try {
            ...
            if ($rgExists -eq "true") {
                ...
                if ($LASTEXITCODE -eq 0) {
                    ...
                    Write-Host "   ❌ Failed to delete Resource Group`n" -ForegroundColor Red
                }
            }
            else {
                Write-Host "   ℹ️  Resource Group not found (may have been already deleted)`n" -ForegroundColor Yellow
            }
        }
        catch {
...
if ($config -and $config.CertificateThumbprint) {
    Write-Host "🗑️  Removing certificate..." -ForegroundColor Yellow
    Write-Host "   Thumbprint: $($config.CertificateThumbprint)" -ForegroundColor Gray
    Write-Host "   Subject: $($config.CertificateSubject)" -ForegroundColor Gray
    ...
    try {
        ...
        if ($cert) {
            ...
            Write-Host "   ℹ️  Certificate not found in Windows Store (may have been already deleted)`n" -ForegroundColor Yellow
        }
    }
    catch {
    ...
    Write-Host "ℹ️  No certificate thumbprint in config. Skipping certificate cleanup.`n" -ForegroundColor Yellow
}
...
if ($config -and $config.CertificateFileName) {
    ...
    if (Test-Path $pfxFile) {
        try {
            ...
            Write-Host "❌ Failed to delete: $($config.CertificateFileName).pfx" -ForegroundColor Red
        }
    }
    else {
        Write-Host "ℹ️  File not found: $($config.CertificateFileName).pfx" -ForegroundColor Yellow
    }
    ...
    if (Test-Path $cerFile) {
        try {
            ...
            Write-Host "❌ Failed to delete: $($config.CertificateFileName).cer" -ForegroundColor Red
        }
    }
    else {
        Write-Host "ℹ️  File not found: $($config.CertificateFileName).cer" -ForegroundColor Yellow
    }
}
...
if (Test-Path $ConfigPath) {
    try {
        ...
        Write-Host "❌ Failed to delete: config.json" -ForegroundColor Red
    }
}
else {
    Write-Host "ℹ️  File not found: config.json" -ForegroundColor Yellow
}
