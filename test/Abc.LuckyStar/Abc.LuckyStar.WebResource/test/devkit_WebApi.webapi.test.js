define(['xrm-mock', 'sinon'], function (/** @type {XrmMock} */_xrm_mock, /** @type {sinon} */_sinon) {
    describe("devkit_WebApi.test", function () {
        ...
        function getFromUserId() {
            ...
            var req = new LuckyStar.WebApi.RetrieveRequest();
            req.entityName = "systemuser";
            req.fetchXml = fetchXml;
            var res = WebApiClient.Retrieve(req);
            var user = new LuckyStar.SystemUserApi(res.value[0]);
            return user.SystemUserId.Value;
        }

        function getToContactId() {
            var key = new LuckyStar.WebApi.AlternateKey("emailaddress1", "someone_a@example.com");
            var req = new LuckyStar.WebApi.RetrieveRequest();
            req.alternateKey = [key];
            req.entityName = "contact";
            req.queryParams = "?$select=contactid";
            var res = WebApiClient.Retrieve(req);
            var contact = new LuckyStar.ContactApi(res);
        ...
        function getToAccountId() {
            ...
            req.fetchXml = fetchXml;
            var res = WebApiClient.Retrieve(req);
            var account = new LuckyStar.AccountApi(res.value[0]);
        ...
        describe("Retrieve", function () {
            it("Retrieve OptionSet", function () {
                ...
                var res = WebApiClient.Retrieve(req);
                //result
                var webapi = new LuckyStar.devkit_WebApiApi(res.value[0]);
            ...
            it("Retrieve Alternate Key", function () {
                ...
                //run
                var key = new LuckyStar.WebApi.AlternateKey("devkit_alternatekey", "KEY-0001");
                var req = new LuckyStar.WebApi.RetrieveRequest();
                req.alternateKey = [key];
                req.entityName = "devkit_webapi";
                var res = WebApiClient.Retrieve(req);
                //result
                var webapi = new LuckyStar.devkit_WebApiApi(res);
        ...
        describe("Insert", function () {
            ...
            it("Insert with Activity Party", function () {
                ...
                var res = WebApiClient.Create(req);
                var email = new LuckyStar.EmailApi(res);
                //send email
                var customRequest = new LuckyStar.WebApi.CustomRequest();
        ...
        describe("Delete", function () {
            it("Delete Contact", function () {
                ...
                //run
                var deleteRequest = new LuckyStar.WebApi.DeleteRequest();
